...
| 参数 | 取值 | 默认 |
|------|------|------|
| `-Preset` | `windows-vs2022`、`windows-vs2022-ci` | `windows-vs2022` |
...
```bash
.\tools\dev\setup.ps1                        # 默认 windows-vs2022
.\tools\dev\setup.ps1 -Preset windows-vs2022-ci
```

或使用仓库根目录脚本（推荐 PowerShell）：
```powershell
.\setup.ps1
.\setup.ps1 ci
.\setup.ps1 deps
```
（`deps` 仅拉子模块与 packman，不跑 CMake，适合只需工具链时。`setup.bat` 为兼容 cmd 的薄包装，等价调用上述命令。）
...
| windows-vs2022-ci | Debug | `build/windows-vs2022-ci/bin/Debug/` |
| windows-vs2022-ci | Release | `build/windows-vs2022-ci/bin/Release/` |
