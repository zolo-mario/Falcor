...
void D3D12HelloFrameBuffering::onLoad(RenderContext* pRenderContext)
{
    // Match D3D12 Hello Frame Buffering: aspect ratio for vertex positions (default 1280/720)
    ...
    // Match D3D12 Hello Frame Buffering vertex data (1:1) - same triangle as HelloTriangle
    ...
    // Program: VS + PS - match D3D12 PSO (frame buffering, fences, command allocators abstracted by Falcor)
    ProgramDesc desc;
    desc.addShaderLibrary("Samples/Desktop/D3D12HelloFrameBuffering/HelloFrameBuffering.slang").vsEntry("VSMain").psEntry("PSMain");
...
void D3D12HelloFrameBuffering::onShutdown() {}

void D3D12HelloFrameBuffering::onResize(uint32_t width, uint32_t height)
{
    // Original D3D12 sample doesn't update vertices on resize; viewport/scissor handled by Falcor
}

void D3D12HelloFrameBuffering::onFrameRender(RenderContext* pRenderContext, const ref<Fbo>& pTargetFbo)
{
    // Match D3D12 Hello Frame Buffering clear color (0.0f, 0.2f, 0.4f, 1.0f)
...
void D3D12HelloFrameBuffering::onGuiRender(Gui* pGui)
{
    Gui::Window w(pGui, "D3D12 Hello Frame Buffering", {250, 200});
    renderGlobalUI(pGui);
    w.text("D3D12 Hello Frame Buffering - migrated to Falcor");
    w.text("Frame buffering (fences, command allocators) abstracted by Falcor");
}

bool D3D12HelloFrameBuffering::onKeyEvent(const KeyboardEvent& keyEvent)
{
    return false;
}

bool D3D12HelloFrameBuffering::onMouseEvent(const MouseEvent& mouseEvent)
{
    return false;
}

void D3D12HelloFrameBuffering::onHotReload(HotReloadFlags reloaded)
{
    //
}
