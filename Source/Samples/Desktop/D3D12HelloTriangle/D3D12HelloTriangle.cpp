...
D3D12HelloTriangle::D3D12HelloTriangle(const SampleAppConfig& config) : SampleApp(config) {}
...
void D3D12HelloTriangle::onLoad(RenderContext* pRenderContext)
{
    ...
    ProgramDesc desc;
    desc.addShaderLibrary("Samples/Desktop/D3D12HelloTriangle/HelloTriangle.slang").vsEntry("VSMain").psEntry("PSMain");
...
int runMain(int argc, char** argv)
{
    ...
    config.windowDesc.title = "D3D12 Hello Triangle";
    config.windowDesc.resizableWindow = true;

    D3D12HelloTriangle project(config);
    return project.run();
}
