...
SimplePostFX 使用 **3 个独立的 ComputePass** 实例，每个负责一个阶段：

1. **mpDownsamplePass** - 入口点：`downsample`
   - 使用 `blurFilter5x5()` 实现 10x10 双线性采样核（binomial kernel）
   - 降采样步长：4x 面积（各维度 2x）

2. **mpUpsamplePass** - 入口点：`upsample`
   - 使用 `blurFilter3x3()` 实现 6x6 双线性采样核
   - 线性混合：`lerp(current, upsampled, gBloomAmount)`
   - 可选星芒效果：6个方向的线采样（level 1-2）

3. **mpPostFXPass** - 入口点：`runPostFX`
   - 应用所有镜头效果和颜色校正
   - 最终输出到 RenderGraph 管理的 `pDst`
...
### 6.1 Wipe 效果实现
- C++ 端计算：`gWipe = mWipe * resolution.x`
- Shader 端判断：`if (pixelPos.x < gWipe) output = gSrc[pixelPos];`
- 实现：在最终输出时，左侧像素直接显示原始输入（无效果），右侧显示后处理结果

### 6.2 金字塔管理策略
- **动态调整**：`preparePostFX()` 根据分辨率和 BloomAmount 动态创建/销毁金字塔
- **内存优化**：BloomAmount <= 0 时所有金字塔纹理设为 nullptr，避免占用内存
- **格式选择**：RGBA16Float 平衡精度和内存使用

### 6.3 星芒效果限制
- 仅在金字塔 level 1 和 2 应用（分辨率约为原大小的 1/4 和 1/16）
- 原因：这些分辨率下星芒细节可见且性能可接受
...
- `gWipe`：从 [0,1] 范围映射到像素坐标

### 6.5 颜色空间处理
- 使用 `luminance()` 函数计算亮度（来自 `ColorHelpers.slang`）
- 基于 ASC-CDL 模型：`pow(max(0, col * scale + offset), power)`
- 饱和度根据亮度分区（阴影/中间调/高光）分别调整
